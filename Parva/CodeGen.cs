  ...
  class CodeGen {
    ...
    // and push Boolean value A op B
	  if(type== Types.stringType){
		  switch (op) {
        case CodeGen.ceq:  Emit(PVM.sceq); break;
        case CodeGen.cne:  Emit(PVM.scne); break;
    ...
    // and store it at the address found on top of stack
      switch (type) {
		...
		case Types.stringType: Emit(PVM.inps); break;
      }
    } // CodeGen.Read
    ...
    // Generates code to output value of specified type from top of stack
      switch (type) {
		...
		case Types.stringType: Emit(PVM.prnvs); break;
      }
    } // CodeGen.Write
	...
	public static void AddString(string stringValue){
		PVM.stringPoolList.Add(stringValue);
	}
  } // end CodeGen
