...
// K.L. Bradshaw, Rhodes University, 2018
...
namespace Parva {
  ...
  // Memory
	...
	public static List<string> stringPoolList = new List<string>();
  ...
  // The interpreters and utility methods
      ...
      do {
        ...
        switch (cpu.ir) {         // execute
          ...
          case PVM.retv:          // return from void function
            ...
            break;
          case PVM.prnvs:
		   results.Write(stringPoolList[Pop()]);
		   break;
		  case PVM.inps:
		  adr= Pop();
		 
		 //  int index= mem[adr]; //index in stringPoolList
		  // Console.WriteLine(index);
		////   if(index < stringPoolList.Count) //if it's not in the string pool
		//		stringPoolList[index] = data.ReadLine();
    //   else  {       
				mem[adr] = stringPoolList.Count;
				stringPoolList.Add(data.ReadLine());
	//	}
			
		  break;
		  case PVM.sceq:
		  //  Console.WriteLine(stringPoolList[Pop()]);
			//Console.WriteLine(stringPoolList[Pop()]);
		    Push(stringPoolList[Pop()].Equals(stringPoolList[Pop()]) ? 1 : 0);
		  break;
		  case PVM.scne:
			Push(!stringPoolList[Pop()].Equals(stringPoolList[Pop()]) ? 1 : 0);
		  break;
		  case PVM.upp:
			adr = Pop();
			stringPoolList.Add(stringPoolList[adr].ToUpper());
			Push(stringPoolList.Count-1);
		  break;
    ...
    // Initializes stack machine
	  stringPoolList = new List<string>();
	  ...
	  mnemonics[PVM.prnvs]    = "PRNVS";
	  mnemonics[PVM.inps]     = "INPS";
	  mnemonics[PVM.sceq]     = "SCEQ";
	  mnemonics[PVM.scne]     = "SCNE";
	  mnemonics[PVM.upp]      = "UPP";
